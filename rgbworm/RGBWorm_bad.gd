...
var current_image = PoolByteArray()
var consumed_colors = {}
var consumed_pixels = {}
var active_worms = []
...
export(Color) var background_color = Color(1, 1, 1, 0) #Hidden maybe?
...
#Canvas size
export(Vector2) var custom_size = Vector2(800, 600) #Negative number means autosize!
var canvas_size
...
func _setup_canvas():
	...
	var r = int(background_color.r * color_steps)
	var g = int(background_color.g * color_steps)
	var b = int(background_color.b * color_steps)
	var a = int(background_color.a * color_steps)
	var pixel = PoolByteArray()
	pixel.append(r)
	pixel.append(g)
	pixel.append(b)
	pixel.append(a)
	
	var pixel_row = PoolByteArray()
	for x in range(canvas_size[0]):
		pixel_row.append_array(pixel)
	
	for y in range(canvas_size[1]):
		print("ROW ", y)
		current_image.append_array(pixel_row)
		
	pass
	print("Done setting up image")
...
func _consume(x, y, red, green, blue):
	...
	#Update the image
	var start_pos = x + (canvas_size[1] * y)
	current_image.set(start_pos + 0, red)
	current_image.set(start_pos + 1, green)
	current_image.set(start_pos + 2, blue)
	current_image.set(start_pos + 3, 1)
...
func _draw():
	...
	for x in canvas_size[0]:
		for y in canvas_size[1]:
			#Get the pixel
			var start_pos = x + (canvas_size[1] * y)
			var r = current_image[start_pos + 0] / color_steps
			var g = current_image[start_pos + 1] / color_steps
			var b = current_image[start_pos + 2] / color_steps
			var a = current_image[start_pos + 3] / color_steps
			
			#Draw a square there
			draw_rect(Rect2(x, y, 1, 1), Color(r, g, b, a))
